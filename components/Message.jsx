...
const Message = ({role, content}) => {
  ...
  return (
    ...
    <div className={`group relative flex max-w-2xl py-3 rounded-xl ${role === 'user' ? 'bg-gray-200 px-5' : 'gap-3'}`}> {/* Couleur de fond du message utilisateur modifiée pour le thème clair */}
                ...
                <div className='flex items-center gap-2 opacity-70'>
                    {
                        role === 'user' ? (
                            <>
                            <Image onClick={copyMessage} src={assets.copy_icon} alt='' className='w-4 cursor-pointer invert'/> {/* Icône inversée pour fond clair */}
                            <Image src={assets.pencil_icon} alt='' className='w-4.5 cursor-pointer invert'/> {/* Icône inversée pour fond clair */}
                            </>
                        ):(
                            <>
            ...
            {/* Couleur du texte du message utilisateur modifiée pour le thème clair */}
            {role === 'user' ? (
